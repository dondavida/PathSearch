...
graph = delaunay(seed=1, size=100)
graph.node_ordering()
...
def get_step_state_of_min_value(dic, states_visited):
    ...
    if len(new_dic.keys()) == 0:
        ...
        return k
    # choose min distance value from the new dictionary of non-visited states
    else:
        min_val = min(new_dic.values())
        key = [k for k, v in new_dic.items() if v == min_val]
        return key[0]

def get_start_state_of_min_value(dic):
...
class DelaunayAgent(BaseAgent):
    ...
    def agent_start(self, start_state):
        ...
        if np.random.rand() < self.epsilon:
            ...
            action_next_state = get_start_state_of_min_value(Q_dictionary[(start_state[0],start_state[1])])
    ...
    def agent_step(self, reward, next_state):
        ...
        delta  = float(reward) + self.discount_factor * min(Q_dictionary[(self.action_next_state[0],self.action_next_state[1])].values()) - current_value
        ...
        if np.random.rand() < self.epsilon:
            ...
            action_next_state = get_step_state_of_min_value(Q_dictionary[(next_state[0],next_state[1])], self.states_visited)
        #action_next_state = get_step_state_of_min_value(Q_dictionary[(next_state[0],next_state[1])], self.states_visited)
        self.states_visited.append(action_next_state)
...
print(states[20][1000])
print(states[50][1000])
#states_visited_num = num_state_occurence(states[50][1000])
#print(states_visited_num)
